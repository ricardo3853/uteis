			...
			<div id="content" class="container <?php echo esc_attr( virtue_container_class() ); ?>">
				<div class="row">
					<div class="main <?php echo esc_attr( virtue_main_class() );?> <?php echo esc_attr( $postclass ) .' '. esc_attr( $fullclass ); ?>" role="main">
		   			 ...
		   			 //$loop = new WP_Query( array( 'post_type' => 'amor' ) );
						if ( $wp_query ) : 
							while ( $wp_query->have_posts() ) : $wp_query->the_post();
						 		if( $summery == 'full' ) {
										get_template_part( 'admin/templates/amor/content', 'fullpost' ); 
								} else {
										get_template_part( 'admin/templates/amor/content', get_post_format() ); 
								}
							endwhile; 
						else: ?>
